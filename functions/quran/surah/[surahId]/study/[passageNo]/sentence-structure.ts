// Sentence structure tab only
// GET /quran/surah/:surahId/study/:passageNo/sentence-structure
...
SELECT json_object(
  ...
  'task', (
    ...
    WHERE t.task_type = 'sentence_structure'
    LIMIT 1
  )
) AS sentence_structure_json
`;
...
export const onRequestGet: PagesFunction<Env> = async (ctx) => {
  ...
  try {
    ...
    const row = await ctx.env.DB.prepare(SQL).bind(surahId, passageNo, containerId, surahUnitId).first<{ sentence_structure_json: string }>();
    if (!row?.sentence_structure_json) {
      return Response.json({ ok: false, error: 'Passage not found' }, { status: 404 });
    }
    const data = JSON.parse(row.sentence_structure_json);
