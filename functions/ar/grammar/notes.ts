...
export const onRequestGet: PagesFunction<Env> = async (ctx) => {
  try {
    ...
    const { results } = await ctx.env.DB.prepare(
      `
      SELECT id, lesson_id, text_type, source, label, note, examples, created_at, updated_at
      FROM grammar_notes
      WHERE lesson_id = ?1
      ORDER BY id ASC
      `
  ...
  try {
    ...
    for (const note of normalized) {
      statements.push(
        ctx.env.DB.prepare(
          `
          INSERT INTO grammar_notes (lesson_id, text_type, source, label, note, examples)
          VALUES (?1, ?2, ?3, ?4, ?5, ?6)
          `
        ).bind(
          lessonId,
