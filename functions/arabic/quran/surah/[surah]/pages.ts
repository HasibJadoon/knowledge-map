...
export const onRequestGet: PagesFunction<Env> = async (ctx) => {
  ...
  const surahNo = Number.parseInt(String(ctx.params?.surah ?? ''), 10);
  if (!Number.isFinite(surahNo) || surahNo < 1 || surahNo > 114) {
    return new Response(JSON.stringify({ ok: false, error: 'Surah number must be between 1 and 114.' }), {
      status: 400,
      headers: jsonHeaders,
    });
  }
  ...
  try {
    const surahStmt = ctx.env.DB.prepare(
      ...
      SELECT
        ...
        s.ayah_count,
        s.meta_json,
        MIN(a.page) AS start_page,
        MAX(a.page) AS end_page,
        MIN(a.juz) AS start_juz
      FROM ar_quran_surahs s
      LEFT JOIN ar_quran_ayah a
        ON a.surah = s.surah
      WHERE s.surah = ?1
      GROUP BY s.surah, s.name_ar, s.name_en, s.ayah_count, s.meta_json
    `
    );
    ...
    const [surahRow, maxPageRow] = await Promise.all([
      surahStmt.bind(surahNo).first<SurahRangeRow>(),
      maxPageStmt.first<MaxPageRow>(),
    ]);

    if (!surahRow || surahRow.start_page == null || surahRow.end_page == null) {
      return new Response(JSON.stringify({ ok: false, error: `Surah ${surahNo} not found.` }), {
        status: 404,
        headers: jsonHeaders,
      });
    }
    ...
    const verseStmt = ctx.env.DB.prepare(
      ...
      LEFT JOIN ar_quran_translations t
       ...
       AND t.ayah = a.ayah
      WHERE a.surah = ?1
      ORDER BY a.page ASC, a.ayah ASC
    `
    );
    ...
    const layoutStmt = ctx.env.DB.prepare(
      ...
      WHERE page_number BETWEEN ?1 AND ?2
      ORDER BY page_number ASC, line_number ASC
    `
    );

    const [{ results: verseRowsRaw = [] }, { results: wordRowsRaw = [] }, layoutResult] = await Promise.all([
      verseStmt.bind(surahNo).all<VerseRow>(),
      wordStmt.bind(surahNo).all<WordRow>(),
      layoutStmt.bind(surahRow.start_page, surahRow.end_page).all<LayoutRow>().catch(() => ({ results: [] as LayoutRow[] })),
    ]);
    ...
    const versesByPage = new Map<number, VerseRow[]>();
    for (const row of verseRows) {
      if (row.page == null) continue;
      const pageVerses = versesByPage.get(row.page) ?? [];
      pageVerses.push(row);
      versesByPage.set(row.page, pageVerses);
    }

    const wordsByPage = new Map<number, WordRow[]>();
    for (const row of wordRows) {
      if (row.page == null) continue;
      const pageWords = wordsByPage.get(row.page) ?? [];
      pageWords.push(row);
      wordsByPage.set(row.page, pageWords);
    }

    const layoutByPage = new Map<number, LayoutRow[]>();
    for (const row of layoutRows) {
      const pageLayout = layoutByPage.get(row.page_number) ?? [];
      pageLayout.push(row);
      layoutByPage.set(row.page_number, pageLayout);
    }

    const pageNumbers = Array.from(versesByPage.keys()).sort((a, b) => a - b);
    const pages = pageNumbers.map((pageNumber, pageIndex) => {
      const pageVerseRows = versesByPage.get(pageNumber) ?? [];
      const pageWordRows = wordsByPage.get(pageNumber) ?? [];
      const verseByKey = new Map(pageVerseRows.map((row) => [`${row.surah}:${row.ayah}`, row] as const));

      const verses = pageVerseRows.map((row) => {
      ...
      const layoutLines = (layoutByPage.get(pageNumber) ?? [])
        .map((row) => {
          const lineType = normalizeLayoutLineType(row.line_type);

          if ((lineType === 'surah_name' || lineType === 'basmallah') && row.surah_number != null && row.surah_number !== surahNo) {
            return null;
          }
          ...
          if (lineType === 'surah_name') {
            text = buildSurahLineText(surahRow.name_ar ?? null);
            textSimple = text || null;
          } else if (lineType === 'basmallah') {
            if (row.surah_number == null && !pageVerseRows.some((verse) => verse.ayah === 1)) {
              return null;
            }
      ...
      const juzs = Array.from(
        new Set(pageVerseRows.map((row) => row.juz).filter((value): value is number => value != null))
      ).sort((a, b) => a - b);
      const hizbs = Array.from(
        new Set(pageVerseRows.map((row) => row.hizb).filter((value): value is number => value != null))
      ).sort((a, b) => a - b);

      return {
        page: {
          number: pageNumber,
          prev_page: pageIndex > 0 ? pageNumbers[pageIndex - 1] ?? null : null,
          next_page: pageIndex < pageNumbers.length - 1 ? pageNumbers[pageIndex + 1] ?? null : null,
          verse_count: verses.length,
          start_ref: `${pageVerseRows[0]!.surah}:${pageVerseRows[0]!.ayah}`,
          end_ref: `${pageVerseRows[pageVerseRows.length - 1]!.surah}:${pageVerseRows[pageVerseRows.length - 1]!.ayah}`,
          juzs,
          hizbs,
        },
        verses,
    ...
    return new Response(
      JSON.stringify({
        ...
        surah: {
          ...
          start_page: surahRow.start_page ?? null,
          end_page: surahRow.end_page ?? null,
          start_juz: surahRow.start_juz ?? null,
          meta: summarizeSurahMeta(surahRow.meta_json),
        },
        stats: {
          total_pages: maxPageRow?.max_page ?? 0,
    ...
    console.error('surah page reader load error', err);
    return new Response(JSON.stringify({ ok: false, error: 'Failed to load surah pages.' }), {
      status: 500,
      headers: jsonHeaders,
    });
  }
