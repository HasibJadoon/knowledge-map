...
export class VocabularyService {
  ...
  async list(params: Record<string, string> = {}) {
    ...
    const url = query ? `/vocabulary?${query}` : this.baseUrl;
  ...
  async update(id: number | string, payload: unknown) {
    return this.request(`/vocabulary/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
  }

  async remove(id: number | string) {
    return this.request(`/vocabulary/${id}`, { method: 'DELETE' });
  }
