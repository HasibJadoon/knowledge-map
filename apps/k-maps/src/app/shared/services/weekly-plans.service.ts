...
export class WeeklyPlansService {
  ...
  private readonly baseUrl = `${API_BASE}/sp_weekly_plans`;
  ...
  async get(id: number | string) {
    return this.request(`/sp_weekly_plans/${id}`);
  }
  ...
  async update(id: number | string, payload: unknown) {
    return this.request(`/sp_weekly_plans/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
  }

  async remove(id: number | string) {
    return this.request(`/sp_weekly_plans/${id}`, { method: 'DELETE' });
  }
