...
export class TafseerPageComponent implements OnInit, AfterViewInit {
  ...
  // Tafsir entries for selected work — surah-browsing
  ...
  setSurah(surah: number): void { this.selectedSurah.set(surah); }
  ...
  private loadEntries(workId: string, surah: number): void {
    ...
    this.api.getTafsirEntries(surah, undefined, workId, 1, 100).subscribe({
      next: res => {
