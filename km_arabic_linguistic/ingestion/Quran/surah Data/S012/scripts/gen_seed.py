...
CLAIMS_ROOT    = Path(__file__).resolve().parents[1] / "claims"  # claims/R*/layer/
...
def gen_al_claims(layer: str, claims_root: Path):
    # Collect from all ruku sub-folders: R*/layer/12_*.json
    files = sorted(claims_root.glob(f"R*/{layer}/12_*.json"),
...
if __name__ == "__main__":
    ...
    gen_al_taxonomy()
    gen_al_claims("irab",    CLAIMS_ROOT)
    gen_al_claims("balagha", CLAIMS_ROOT)
    print(f"\nDone. Files in {OUT}")
