...
CLAIMS_DIR = Path(__file__).resolve().parents[1] / "claims/balagha"  # s12/claims/balagha/
SQL_OUT    = Path(__file__).resolve().parents[1] / "sql/s12_al_claims_balagha.sql"
...
def call_llm(ayah: int, words: list[dict], source_block: str, sources_used: list[str]) -> dict:
    ...
    try:
        with urllib.request.urlopen(req, timeout=720) as r:
...
def write_claim_file(ayah: int, result: dict, sources_used: list[str]):
    ...
    (CLAIMS_DIR / f"12_{ayah}.json").write_text(
