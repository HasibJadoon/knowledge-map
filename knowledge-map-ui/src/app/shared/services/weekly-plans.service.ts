...
export class WeeklyPlansService {
  ...
  private readonly baseUrl = '/weekly_plans';

  async list(params: Record<string, string> = {}) {
    const query = new URLSearchParams(params).toString();
    const url = query ? `/weekly_plans?${query}` : this.baseUrl;
    return this.request(url);
  }

  async get(id: number | string) {
    return this.request(`/weekly_plans/${id}`);
  }
  ...
  async update(id: number | string, payload: unknown) {
    return this.request(`/weekly_plans/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
  }

  async remove(id: number | string) {
    return this.request(`/weekly_plans/${id}`, { method: 'DELETE' });
  }
