...
UPDATE ar_ling_lexicon_blocks SET text_html='<p>Here <span dir="rtl" lang="ar">زُلْفَىٰ</span> is in the accusative (<span dir="rtl" lang="ar">منصوب</span>), though grammarians differ on its role. Sībawayh — as Ibn ʿAṭiyya reports in <em>al-Muḥarrar</em> — reads it as a <em>state-word</em>: the sense is as if it read "drawing near" (<span dir="rtl" lang="ar">مُتَزَلِّفِينَ</span>), describing <em>how</em> the idolaters picture their approach, driven by the verb "they bring us near" (<span dir="rtl" lang="ar">تُقَرِّبُونَا</span>). Others take it as an <em>echo-object</em> that simply intensifies that verb''s meaning. Either way the case-mark is not even written here — it is an implied vowel the long final letter cannot bear.</p>' WHERE id='blk_kmaps_zlf_irab';
